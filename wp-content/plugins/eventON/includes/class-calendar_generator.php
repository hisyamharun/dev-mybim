	...
	/** MAIN function to generate individual ONLY events HTML */	 
		public function eventon_generate_events($args){
			...
			if(!empty($ecv['sep_month']) && $ecv['sep_month']=='yes' && $ecv['number_of_months']>1 ){
				...
				$isEventCount = ( $ecv['event_count']>0)? $ecv['event_count']:false;

				// if events exist
				if(sizeof($event_list_array)){
					$sorted_list = array();
					...
					$content .= $this->evo_process_event_list_data($event_list_array);
				}
		...
		public function wp_query_event_cycle(
			...
			if ( $events->have_posts() ) :
				...
				// override past event cut-off
					...
					// current year month
						$__current_year = date('Y', $focus_month_beg_range);
						$__current_month = date('n', $focus_month_beg_range);
				...
				*/					
				...
				// SCHEME SEO
					...
					if(!empty($this->evopt1['evo_schema']) && $this->evopt1['evo_schema']=='yes'){// remove schema
						...
						$event_permalink = ($_sin_ev_ex)? $event_permalink.$_link_text: get_permalink($event_id);
						$__scheme_data = 
